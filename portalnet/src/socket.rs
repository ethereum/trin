...
pub fn upnp_for_external(listen_addr: SocketAddr) -> Option<SocketAddr> {
    ...
    match gateway.add_any_port(igd_next::PortMappingProtocol::UDP, local_addr, UPNP_MAPPING_DURATION, "new_port") {
        Ok(port) => {
            thread::spawn(move || {
                loop {
                    thread::sleep(time::Duration::from_secs(UPNP_MAPPING_TIMEOUT));
                    match gateway.add_port(igd_next::PortMappingProtocol::UDP, port, local_addr, UPNP_MAPPING_DURATION, "renew_port") {
                        Err(err) => {

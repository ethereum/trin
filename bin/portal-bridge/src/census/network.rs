...
impl Network {
    ...
    async fn liveness_check(&self, enr: Enr) -> LivenessResult {
        ...
        let radius = match PingExtension::decode_json(pong_info.payload_type, pong_info.payload) {
            ...
            _ => {
                warn!(
                    subnetwork = %self.subnetwork,
                    "liveness_check: received unsupported ping extension: {}", pong_info.payload_type,
                );
                return LivenessResult::Fail;
            }
        };
    ...
    async fn ping(&self, enr: &Enr) -> anyhow::Result<PongInfo> {
        ...
        match self.subnetwork {
            Subnetwork::History => HistoryNetworkApiClient::ping(&self.client, enr.clone(), None),
            Subnetwork::State => StateNetworkApiClient::ping(&self.client, enr.clone(), None),
            Subnetwork::Beacon => BeaconNetworkApiClient::ping(&self.client, enr.clone(), None),

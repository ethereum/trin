...
// This struct provides various era files based on the period and makes
// them accessible via a specific block number from that period.
// - pre-merge: all periods will correspond 1:1 with era1 files
// - merge-boundary: will have an era1 & 2 era files
// - post-merge: all periods will likely have 2 era files
pub struct EraProvider {
    pub sources: Vec<EraSource>,
}

impl EraProvider {
    pub async fn new(period: u64) -> anyhow::Result<Self> {
        info!("Fetching e2store files for period: {period}");
        ...
        let mut previous_era_period_index = None;
        ...
        while next_block < ending_block {
            let source = if !network_spec().is_paris_active_at_block(next_block) {
                ...
                let period_index = next_block / SLOTS_PER_HISTORICAL_ROOT;
                let era1_path = era1_paths.get(&period_index).ok_or(anyhow!(
                    "Era1 file not found for period index: {period_index}",
                ))?;
                let raw_era1 = fetch_bytes(http_client.clone(), era1_path).await?;
                EraSource::PreMerge(Arc::new(Era1::deserialize(&raw_era1)?))
            } else {
                let era = Arc::new(match previous_era_period_index {
                    Some(previous_period_index) => {
                        let era_path = era_links
                            .get(&(previous_period_index + 1))
                        ...
                        previous_era_period_index = Some(era.epoch_index());
                        MinimalEra::from(era)
                    }
                    None => {

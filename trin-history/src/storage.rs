...
impl HistoryStorage {
    ...
    fn total_entry_count(&self) -> Result<u64, ContentStoreError> {
        ...
        let result: Result<Vec<EntryCount>, rusqlite::Error> = query
            ...
            .collect();
        self.metrics.stop_storage_process_timer(timer);
        match result?.first() {
            Some(val) => Ok(val.0),
    ...
    fn store(
        ...
        let store_timer = self.metrics.start_storage_process_timer("store");
        let content_id = key.content_id();
        ...
        self.metrics.stop_storage_process_timer(store_timer);
        self.prune_db()?;
    ...
    fn prune_db(&mut self) -> Result<usize, ContentStoreError> {
        let timer = self.metrics.start_storage_process_timer("prune_db");
    ...
    fn get_content_size(&self, id: &[u8; 32]) -> Result<u64, ContentStoreError> {
        let timer = self.metrics.start_storage_process_timer("get_content_size");
        let conn = self.sql_connection_pool.get()?;
    ...
    fn lookup_content_key(&self, id: [u8; 32]) -> anyhow::Result<Option<Vec<u8>>> {
        let timer = self
            .metrics
            .start_storage_process_timer("lookup_content_key");
        let conn = self.sql_connection_pool.get()?;
    ...
    fn lookup_content_value(&self, id: [u8; 32]) -> anyhow::Result<Option<Vec<u8>>> {
        let timer = self
            .metrics
            .start_storage_process_timer("lookup_content_value");
        let conn = self.sql_connection_pool.get()?;
    ...
    fn get_total_storage_usage_in_bytes_on_disk(&self) -> Result<u64, ContentStoreError> {
        let timer = self
            .metrics
            .start_storage_process_timer("get_total_storage_usage_in_bytes_on_disk");
    ...
    fn db_insert(
        ...
        let timer = self.metrics.start_storage_process_timer("db_insert");
        let conn = self.sql_connection_pool.get()?;
    ...
    fn db_remove(&self, content_id: &[u8; 32]) -> Result<(), ContentStoreError> {
        let timer = self.metrics.start_storage_process_timer("db_remove");
    ...
    fn get_total_storage_usage_in_bytes_from_network(&self) -> Result<u64, ContentStoreError> {
        let timer = self
            .metrics
            .start_storage_process_timer("get_total_storage_usage_in_bytes_from_network");
        let conn = self.sql_connection_pool.get()?;
    ...
    fn find_farthest_content_id(&self) -> Result<Option<[u8; 32]>, ContentStoreError> {
        let timer = self
            .metrics
            .start_storage_process_timer("find_farthest_content_id");

                    ...
                    FindContentQueryPending::Utp {
                        ...
                        tokio::spawn(async move {
                            ...
                            let data = match utp_processing
                                .utp_controller
                                .connect_inbound_stream(cid, EnrPeer(source))
                                .await
                            {
        ...
        match (
            ...
            (Ok(Some(content)), Some(permit)) => {
                if content.len() <= MAX_PORTAL_CONTENT_PAYLOAD_SIZE {
                    ...
                    tokio::spawn(async move {
                        utp.accept_outbound_stream(cid, EnrPeer(enr), &content)
                            .await;
                        permit.drop();
                    });
            ...
            Ok(Response::Content(found_content)) => {
                match found_content {
                    ...
                    Content::ConnectionId(conn_id) => {
                        ...
                        utp_processing
                            .utp_controller
                            .connect_inbound_stream(cid, EnrPeer(fallback_peer.clone()))
                            .await?
                            .into()
                    }
                }

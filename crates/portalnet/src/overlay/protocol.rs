    ...
    > OverlayProtocol<TContentKey, TMetric, TValidator, TStore, TPingExtensions>
{
    ...
    ) -> Result<(Option<u16>, Option<PingExtension>), OverlayRequestError> {
        Ok(match (payload_type, payload) {
            (Some(payload_type), Some(payload)) => {
                ...
                match PingExtension::decode_json(payload_type, payload) {
                    Ok(payload) => (Some(payload_type), Some(payload)),
                    Err(err) => {
            ...
            (None, Some(_)) => {
                return Err(OverlayRequestError::PayloadTypeIsRequired {
                    message: "Payload type not provided".to_string(),
                })
            }
            (Some(payload_type), None) => (Some(payload_type), None),
            (None, None) => (None, None),
        })
    }

    /// Generate a default payload for a respective payload type.
    pub fn default_payload(&self, payload_type: u16) -> Result<CustomPayload, OverlayRequestError> {
        Ok(
            match Extensions::try_from(payload_type).map_err(|_| {
                OverlayRequestError::PayloadTypeNotSupported {
                    message: "Payload type not supported".to_string(),
                    reason: PayloadTypeNotSupportedReason::Client,
                }
            })? {
                Extensions::Capabilities => ClientInfoRadiusCapabilities::new(
                    self.data_radius(),
                ...
                .into(),
                Extensions::Error => return Err(OverlayRequestError::Failure("Can't create error message for ping, the Error payload can only be used by pong".to_string())),
            },
        )
    }
        ...
        let (payload_type, payload) = match (payload_type, payload) {
            (Some(payload_type), Some(payload)) => {
                if payload_type != payload.payload_type() {
                    return Err(OverlayRequestError::PayloadTypeDidntMatchPayload {

...
pub enum OverlayRequestError {
    ...
    #[error("Payload type is required: {message}")]
    PayloadTypeIsRequired { message: String },

    #[error("Payload type didn't match payload: expected {expected}, actual {actual}")]
    PayloadTypeDidntMatchPayload { expected: u16, actual: u16 },
}

...
impl std::fmt::Debug for EnrPeer {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl ConnectionPeer for EnrPeer {
    type Id = NodeId;
...
#[async_trait]
impl AsyncUdpSocket<EnrPeer> for Discv5UdpSocket {
    async fn send_to(&mut self, buf: &[u8], peer: &Peer<EnrPeer>) -> io::Result<usize> {
        let peer = peer.clone();
        ...
        tokio::spawn(async move {
            let enr = match peer.peer() {
                ...
                None => match find_enr(peer.id(), &discv5, enr_cache, header_oracle).await {
                    Ok(enr) => enr,
                    Err(err) => {
    ...
    async fn recv_from(&mut self, buf: &mut [u8]) -> io::Result<(usize, Peer<EnrPeer>)> {

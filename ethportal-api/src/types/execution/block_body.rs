...
mod tests {
    ...
    #[test]
    fn test_rlp_legacy_with_empty_uncles() {
        let block_body_legacy = BlockBodyLegacy {
            txs: vec![rlp::decode(&hex_decode(TX1).unwrap()).unwrap()],
            uncles: vec![],
        };
        let encoded = rlp::encode(&block_body_legacy);
        let decoded: BlockBody = rlp::decode(&encoded).unwrap();
        assert_eq!(decoded, BlockBody::Legacy(block_body_legacy));
    }

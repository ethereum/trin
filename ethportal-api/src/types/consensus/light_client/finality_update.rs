...
impl LightClientFinalityUpdate {
    pub fn from_ssz_bytes(bytes: &[u8], fork_name: ForkName) -> Result<Self, ssz::DecodeError> {
        match fork_name {
            ForkName::Altair => {
                LightClientFinalityUpdateAltair::from_ssz_bytes(bytes).map(Self::Altair)
            }
            ForkName::Bellatrix => {
...
mod test {
    ...
    #[case("case_3")]
    #[case("case_4")]
    fn serde_light_client_finality_update_altair(#[case] case: &str) {
        let value = std::fs::read_to_string(format!(
            "../test_assets/beacon/altair/LightClientFinalityUpdate/ssz_random/{case}/value.yaml"
        ))
        .expect("cannot find test asset");
        let value: Value = serde_yaml::from_str(&value).unwrap();
        let content: LightClientFinalityUpdateAltair =
    ...
    #[case("case_3")]
    #[case("case_4")]
    fn ssz_light_client_finality_update_altair(#[case] case: &str) {
        let value = std::fs::read_to_string(format!(
            "../test_assets/beacon/altair/LightClientFinalityUpdate/ssz_random/{case}/value.yaml"
        ))
        .expect("cannot find test asset");
        let value: Value = serde_yaml::from_str(&value).unwrap();
        let content: LightClientFinalityUpdateAltair = serde_json::from_value(value).unwrap();

        let compressed = std::fs::read(format!(
            "../test_assets/beacon/altair/LightClientFinalityUpdate/ssz_random/{case}/serialized.ssz_snappy"
        ))
            .expect("cannot find test asset");
        let mut decoder = snap::raw::Decoder::new();
        let expected = decoder.decompress_vec(&compressed).unwrap();
        LightClientFinalityUpdate::from_ssz_bytes(&expected, ForkName::Altair).unwrap();

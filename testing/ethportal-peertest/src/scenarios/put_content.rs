...
pub async fn test_gossip_with_trace(peertest: &Peertest, target: &Client) {
    ...
    let _ = HistoryNetworkApiClient::ping(target, peertest.bootnode.enr.clone(), None)
        .await
        .unwrap();
    ...
    // connect to new node
    let _ = HistoryNetworkApiClient::ping(target, fresh_enr, None)
        .await
        .unwrap();
...
pub async fn test_gossip_dropped_with_offer(peertest: &Peertest, target: &Client) {
    ...
    let _ =
        HistoryNetworkApiClient::ping(&fresh_target, target.node_info().await.unwrap().enr, None)
            .await
            .unwrap();
    let _ = HistoryNetworkApiClient::ping(&fresh_target, peertest.bootnode.enr.clone(), None)
        .await
        .unwrap();
    let _ = HistoryNetworkApiClient::ping(&fresh_target, peertest.nodes[0].enr.clone(), None)
        .await
        .unwrap();

...
impl Default for TrinConfig {
    fn default() -> Self {
        TrinConfig {
            web3_transport: Web3TransportType::from_str(DEFAULT_WEB3_TRANSPORT, false)
...
impl TrinConfig {
    ...
    {
        let config = match Self::try_parse_from(args) {
            Ok(config) => config,
            Err(err) => panic!("Paser TrinConfig error: {}", err),
        };
